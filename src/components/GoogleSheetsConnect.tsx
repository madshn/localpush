...
export function GoogleSheetsConnect({ onConnected }: GoogleSheetsConnectProps) {
  ...
  const handleSignIn = async () => {
    if (!GOOGLE_CLIENT_ID) {
      toast.error('Google OAuth not configured. Set client ID in GoogleSheetsConnect.tsx');
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs text-text-secondary">
        Connect your Google account to push data directly to Google Sheets spreadsheets. Each source
        gets its own worksheet tab.
      </p>
      ...
      <div className="flex gap-2.5 p-3 bg-accent/10 border border-accent/20 rounded-md">
        ...
        <div className="text-[11px] text-text-secondary leading-relaxed">
          <p className="font-medium text-text-primary mb-1">Minimal permissions requested:</p>
          <ul className="list-disc list-inside space-y-0.5">
            <li>
              <strong>Drive (read-only)</strong> — list your spreadsheets
            </li>
            <li>
              <strong>Sheets (read/write)</strong> — append rows to bound spreadsheets
            </li>
          </ul>
          <p className="mt-1.5">
            LocalPush never reads existing spreadsheet data. It only appends new rows.
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        ...
        >
          {isSigningIn || connectMutation.isPending ? 'Connecting...' : 'Sign in with Google'}
        </button>
      </div>
