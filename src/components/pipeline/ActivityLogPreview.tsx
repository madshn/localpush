  ...
  if (previewEntries.length === 0) {
    return (
      <div className="bg-bg-secondary border border-border rounded-lg p-3">
        ...
        <p className="text-xs text-text-secondary/60 mt-2">No deliveries yet</p>
      </div>
    );
  }
    ...
    <div className="bg-bg-secondary border border-border rounded-lg p-3">
      ...
      <div className="flex flex-col gap-1.5">
        {previewEntries.map((entry) => {
          const { icon: StatusIcon, className } = statusIcon[entry.status] || statusIcon.pending;
          return (
            <div
              key={entry.id}
            ...
            >
              ...
              <span className="text-[11px] font-medium truncate flex-1">{entry.source}</span>
